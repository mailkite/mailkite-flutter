...
<h1 align="center">mailkite_client</h1>
...
<p align="center">
  ...
  <br>The official <a href="https://mailkite.dev">MailKite</a> library for Flutter.
</p>
...
> **Read-only mirror.** This repo is a generated, release-time mirror of the MailKite monorepo (the private source of truth) — development doesn't happen here. Install from pub.dev and open issues against the [MailKite docs](https://mailkite.dev/docs).

## Install

```sh
flutter pub add mailkite_client
```

## Quickstart

```dart
final mk = mailKiteClient(clientId: '…');

await mk.login();   // flutter_web_auth_2 + flutter_secure_storage
final res = await mk.send(SendMessage(
  from: 'you@yourdomain.com', to: 'x@example.com', subject: 'Hi', text: 'Hello'));
```
...
| --- | --- |
| [`examples/login_and_send.dart`](examples/login_and_send.dart) | Client-side OAuth: a button that signs the end user into THEIR OWN MailKite |
...
| mailkite_client **(this repo)** | [`mailkite-flutter`](https://github.com/mailkite/mailkite-flutter) | pub.dev |
